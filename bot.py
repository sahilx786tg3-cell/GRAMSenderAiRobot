...
BOT_TOKEN = "8977870710:AAH_pRWKfoWnbBteHuobVyNFr7DgDiMqrhw"
...
TONCENTER_API_KEY = "50b38c6fa5198ff9d201521654bc8145456accc93af8d1d34740f247495a37e4"
DB_FILE = "users.json"
...
def handle_all(message):
    ...
    if lower == "/balance":
        try:
            ...
            bot.reply_to(message, f"Wallet load failed!\nReason: `{e}`", parse_mode="Markdown")
            return
        ...
        if balance is None:
            bot.reply_to(
                message,
                f"Could not fetch balance (API error).\n"
                f"Wallet State: `{state}`\n\n"
                f"Address:\n`{addr}`\n\n"
                f"Check server logs for [BALANCE ERROR] details.",
                parse_mode="Markdown"
            )
            return
        bot.reply_to(
